    ...
    public static final <R> Object suspendWithStateAtLeastUnchecked(Lifecycle lifecycle, Lifecycle.State state, boolean z, CoroutineDispatcher coroutineDispatcher, Function0<? extends R> function0, Continuation<? super R> continuation) {
        ...
        if (z) {
            coroutineDispatcher.dispatch(EmptyCoroutineContext.INSTANCE, new Runnable(lifecycle, state, function0, z, coroutineDispatcher) { // from class: androidx.lifecycle.WithLifecycleStateKt$suspendWithStateAtLeastUnchecked$$inlined$suspendCancellableCoroutine$lambda$2
                ...
                {
                    this.$this_suspendWithStateAtLeastUnchecked$inlined = r2;
                    this.$state$inlined = r3;
                    this.$block$inlined = r4;
                    this.$dispatchNeeded$inlined = r5;
                    this.$lifecycleDispatcher$inlined = r6;
                }

                @Override // java.lang.Runnable
                public final void run() {
                    this.$this_suspendWithStateAtLeastUnchecked$inlined.addObserver(WithLifecycleStateKt$suspendWithStateAtLeastUnchecked$$inlined$suspendCancellableCoroutine$lambda$1.this);
                }
            });
        } else {
