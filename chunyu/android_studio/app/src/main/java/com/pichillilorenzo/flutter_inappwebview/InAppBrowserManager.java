...
public class InAppBrowserManager implements MethodChannel.MethodCallHandler {
    ...
    public void onMethodCall(MethodCall methodCall, MethodChannel.Result result) {
        ...
        if (c == 0) {
            openUrl(activity, str, (String) methodCall.argument(ImagesContract.URL), (HashMap) methodCall.argument(Constant.METHOD_OPTIONS), (Map) methodCall.argument("headers"), (HashMap) methodCall.argument("contextMenu"), (Integer) methodCall.argument("windowId"), (Map) methodCall.argument("closeButtonOptions"));
            result.success(true);
        } else if (c == 1) {
            String str3 = (String) methodCall.argument(ImagesContract.URL);
            try {
                openUrl(activity, str, Util.getUrlAsset(str3), (HashMap) methodCall.argument(Constant.METHOD_OPTIONS), (Map) methodCall.argument("headers"), (HashMap) methodCall.argument("contextMenu"), (Integer) methodCall.argument("windowId"));
                result.success(true);
            } catch (IOException e) {
                e.printStackTrace();
                result.error(LOG_TAG, str3 + " asset file cannot be found!", e);
            }
        } else if (c == 2) {
            openData(activity, str, (HashMap) methodCall.argument(Constant.METHOD_OPTIONS), (String) methodCall.argument("data"), (String) methodCall.argument("mimeType"), (String) methodCall.argument("encoding"), (String) methodCall.argument("baseUrl"), (String) methodCall.argument("historyUrl"), (HashMap) methodCall.argument("contextMenu"), (Integer) methodCall.argument("windowId"));
            result.success(true);
        } else if (c != 3) {
