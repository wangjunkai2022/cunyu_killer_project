...
public final class NotificationService extends Service {
        ...
        public final void updatePosition(Context context, boolean z, long j, float f) {
            ...
            if (z2) {
                ...
                if (Intrinsics.areEqual(playbackStateCompat2 == null ? null : Float.valueOf(playbackStateCompat2.getPlaybackSpeed()), f)) {
                    Companion companion = NotificationService.Companion;
            ...
            Companion companion2 = NotificationService.Companion;
            NotificationService.stateCompat = build;
    ...
    private final Intent createReturnIntent(String str, String str2, AudioMetas audioMetas) {
        Intent putExtra = new Intent(this, NotificationActionReceiver.class).setAction(str).putExtra(EXTRA_PLAYER_ID, str2).putExtra(TRACK_ID, audioMetas.getTrackID());
    ...
    private final void displayNotification(NotificationAction.Show show) {
        Job unused = BuildersKt__Builders_commonKt.launch$default(GlobalScope.INSTANCE, Dispatchers.getMain(), null, new NotificationService$displayNotification$1(this, show, null), 2, null);
    }
    ...
    public final void displayNotification(NotificationAction.Show show, Bitmap bitmap) {
        ...
        if (notificationSettings.getPrevEnabled()) {
            builder.addAction(getPrevIcon(notificationService, show.getNotificationSettings().getPreviousIcon()), NotificationAction.ACTION_PREV, PendingIntent.getBroadcast(notificationService, 0, createReturnIntent(NotificationAction.ACTION_PREV, show.getPlayerId(), show.getAudioMetas()), 201326592));
        }
        if (notificationSettings.getPlayPauseEnabled()) {
            builder.addAction(show.isPlaying() ? getPauseIcon(notificationService, show.getNotificationSettings().getPauseIcon()) : getPlayIcon(notificationService, show.getNotificationSettings().getPlayIcon()), show.isPlaying() ? "pause" : "play", broadcast);
        }
        if (notificationSettings.getNextEnabled()) {
            builder.addAction(getNextIcon(notificationService, show.getNotificationSettings().getNextIcon()), NotificationAction.ACTION_NEXT, PendingIntent.getBroadcast(notificationService, 0, createReturnIntent(NotificationAction.ACTION_NEXT, show.getPlayerId(), show.getAudioMetas()), 201326592));
        }
        if (notificationSettings.getStopEnabled()) {
            builder.addAction(getStopIcon(notificationService, show.getNotificationSettings().getStopIcon()), NotificationAction.ACTION_STOP, PendingIntent.getBroadcast(notificationService, 0, createReturnIntent(NotificationAction.ACTION_STOP, show.getPlayerId(), show.getAudioMetas()), 201326592));
        }
        ...
        NotificationCompat.Builder contentIntent = onlyAlertOnce.setContentIntent(PendingIntent.getBroadcast(notificationService, 0, createReturnIntent(NotificationAction.ACTION_SELECT, show.getPlayerId(), show.getAudioMetas()), 335544320));
        if (bitmap != null) {
