...
public final /* synthetic */ class FlowKt__DelayKt {
    ...
    public static final <T> Flow<T> debounce(Flow<? extends T> flow, long j) {
        int i = (j > 0 ? 1 : (j == 0 ? 0 : -1));
        if (i >= 0) {
            return i == 0 ? flow : debounceInternal$FlowKt__DelayKt(flow, new Function1<T, Long>(j) { // from class: kotlinx.coroutines.flow.FlowKt__DelayKt$debounce$2
                final /* synthetic */ long $timeoutMillis;

                /* JADX INFO: Access modifiers changed from: package-private */
                {
                    this.$timeoutMillis = r1;
                }
    ...
    public static final <T> Flow<T> m1458debounce8GFy2Ro(Flow<? extends T> flow, double d) {
        return FlowKt.debounce(flow, DelayKt.m1420toDelayMillisLRDsOJo(d));
    }
    ...
    public static final <T> Flow<T> debounceDuration(Flow<? extends T> flow, Function1<? super T, Duration> function1) {
        return debounceInternal$FlowKt__DelayKt(flow, new Function1<T, Long>() { // from class: kotlinx.coroutines.flow.FlowKt__DelayKt$debounce$3
            ...
            @Override // kotlin.jvm.functions.Function1
            public final Long invoke(T t) {
                return DelayKt.m1420toDelayMillisLRDsOJo(((Duration) Function1.this.invoke(t)).m1383unboximpl());
            }
        });
    }
    ...
    public static final <T> Flow<T> m1459sample8GFy2Ro(Flow<? extends T> flow, double d) {
        return FlowKt.sample(flow, DelayKt.m1420toDelayMillisLRDsOJo(d));
    }
}
