...
public class TPMediaCodecAudioDecoder extends TPBaseMediaCodecDecoder {
    ...
    public boolean initDecoder(String str, int i, int i2, int i3, int i4) {
        TPNativeLog.printLog(2, "TPMediaCodecAudioDecoder", "initDecoder, mimeType:" + str + " sampleRate:" + i + " channelCount:" + i2 + " drmType:" + i3 + " audioFormat:" + i4);
        this.mMimeType = str;
        this.mSampleRate = i;
    ...
    void processOutputFormatChanged(MediaFormat mediaFormat) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT < 24 || !mediaFormat.containsKey("pcm-encoding")) {
                ...
                try {
                    TPNativeLog.printLog(2, "TPMediaCodecAudioDecoder", "processOutputFormatChanged: MediaFormat.KEY_PCM_ENCODING: " + i);
                } catch (Exception e2) {
                    e = e2;
                    i = i;
                    TPNativeLog.printLog(4, "TPMediaCodecAudioDecoder", "processOutputFormatChanged got one exception: " + getStackTrace(e));
                    TPNativeLog.printLog(2, "TPMediaCodecAudioDecoder", "processOutputFormatChanged, mEnableAudioPassThrough:" + this.mEnableAudioPassThrough + ", mSampleRate: " + this.mSampleRate + ", mChannelCount: " + this.mChannelCount + " mAudioFormat: " + this.mAudioFormat + " pcmFormat:" + i);
                }
            }
        } catch (Exception e3) {
            e = e3;
