...
public class PluginManger {
    ...
    public void unLoadPlugin() {
        for (PluginInfo pluginInfo : this.mPluginConfigList) {
            ...
            Log.w("HostEngine-PluginManger", "[unLoadPlugin], unLoadPlugin=" + i);
    ...
    private void doLoadPlugin() {
        for (PluginInfo pluginInfo : this.mPluginConfigList) {
            TXCLog.d("HostEngine-PluginManger", "[loadPlugin], pluginId=" + pluginInfo.mPluginId + " ,pluginClazzName=" + pluginInfo.mPluginClazzName);
            if (pluginInfo.mIsCorePlugin) {
                _doLoadPlugin(pluginInfo);
            } else {
                TXCLog.d("HostEngine-PluginManger", "[loadPlugin], pluginId=" + pluginInfo.mPluginId + " is not core plugin, do not load by default");
            }
        }
    }
    ...
    private boolean _doLoadPlugin(PluginInfo pluginInfo) {
        int i = pluginInfo.mPluginId;
        if (this.mPluginMap.containsKey(Integer.valueOf(i))) {
            TXCLog.w("HostEngine-PluginManger", "[loadPlugin], pluginId has been loaded!!, pluginId=" + i);
            return true;
        }
        ...
        if (createPluginInstance != null) {
            ...
            TXCLog.d("HostEngine-PluginManger", "[loadPlugin], succeed loading pluginId=" + i + " ,pluginClazzName=" + pluginInfo.mPluginClazzName);
            return true;
        }
        TXCLog.w("HostEngine-PluginManger", "[loadPlugin], pluginId=" + i + " is not exist, do not load!!");
        return false;
    }

    private IPluginBase createPluginInstance(int i, String str) {
        Log.i("HostEngine-PluginManger", "[createPluginInstance],pluginId|" + i + "|clazzName|" + str);
        ...
        try {
            ...
            Log.e("HostEngine-PluginManger", "create pluginInstance exception, pluginId|" + i + "|clazzName|" + str + " is not install in dex!!");
            return null;
        }
    }
