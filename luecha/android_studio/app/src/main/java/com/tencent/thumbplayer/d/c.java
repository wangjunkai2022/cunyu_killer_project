...
public class c implements a, f.a, i.b {
    ...
    private boolean k() {
        ...
        try {
            ...
            if (!TextUtils.isEmpty(TPPlayerConfig.getUserUin())) {
                this.c.setUserData("user_uin", TPPlayerConfig.getUserUin());
            }
            ...
            this.c.setUserData("carrier_pesudo_code", TPPlayerConfig.getUserUpc());
            this.c.setUserData("carrier_pesudo_state", Integer.valueOf(TPPlayerConfig.getUserUpcState()));
            this.c.setUserData("external_network_ip", TPPlayerConfig.getOutNetIp());
            return true;
        } catch (Throwable th) {
            TPLogUtil.e("TPThumbPlayer[TPPlayManagerImpl.java]", th);
            return false;
    ...
    private void q() {
        try {
            this.c.setUserData("dl_param_play_start_time", Long.valueOf(this.q));
            this.c.setUserData("dl_param_play_end_time", Long.valueOf(this.r));
        } catch (Throwable th) {
            TPLogUtil.e("TPThumbPlayer[TPPlayManagerImpl.java]", th);
        }
    }
