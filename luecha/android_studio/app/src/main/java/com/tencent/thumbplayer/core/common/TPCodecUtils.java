...
public class TPCodecUtils {
    ...
    public static boolean addHDRVideoDecoderTypeWhiteList(int i, int i2, TPCodecCapability.TPHdrSupportVersionRange tPHdrSupportVersionRange) {
        ...
        if (i2 == 101) {
            ...
            TPNativeLog.printLog(3, "TPCodecUtils", "addHDRVideoDecoderTypeWhiteList, decoder not support.");
            return false;
        } else {
    ...
    public static int convertDolbyVisionToOmxLevel(int i) {
        int i2 = 1 << i;
        if (i2 < 1 || i2 > 256) {
            TPNativeLog.printLog(2, "TPCodecUtils", "convertDolbyVisionToOmxLevel Unsupported level" + i);
            return i;
        }
        TPNativeLog.printLog(2, "TPCodecUtils", "convertDolbyVisionToOmxLevel dolbyVisionLevel:" + i + " omxLevel:" + i2);
        return i2;
    }
    ...
    public static int convertDolbyVisionToOmxProfile(int i) {
        int i2 = 1 << i;
        if (i < 1 || i > 512) {
            TPNativeLog.printLog(2, "TPCodecUtils", "convertDolbyVisionToOmxProfile Unsupported profile" + i);
            return i;
        }
        TPNativeLog.printLog(2, "TPCodecUtils", "convertDolbyVisionToOmxProfile dolbyVisionProfile:" + i + " omxProfile:" + i2);
        return i2;
    }
    ...
    public static synchronized HashMap<Integer, TPCodecCapability.TPCodecMaxCapability> getACodecSWMaxCapabilityMap() {
        synchronized (TPCodecUtils.class) {
            TPNativeLog.printLog(2, "TPCodecUtils", "getACodecSWMaxCapabilityMap func in");
            ...
            try {
                ...
                TPNativeLog.printLog(4, "TPCodecUtils", "getACodecSWMaxCapabilityMap exception");
                return null;
            }
        }
    }
            ...
            try {
                ...
                for (TPMediaDecoderInfo tPMediaDecoderInfo : tPMediaDecoderInfos) {
                    ...
                    if (tPMediaDecoderInfo.isAudio() && isSupportedMediaCodec(decoderMimeType) && !isInMediaCodecBlackList(decoderMimeType) && !isAMediaCodecBlackListInstance(tPMediaDecoderInfo.getDecoderName())) {
                        TPNativeLog.printLog(2, "TPCodecUtils", "Audio MimeType: " + decoderMimeType + " codecName: " + tPMediaDecoderInfo.getDecoderName());
                        ...
                        if (!mMaxACodecHwCapabilityMap.containsKey(Integer.valueOf(getSupportedCodecId(decoderMimeType)))) {
                            TPNativeLog.printLog(2, "TPCodecUtils", "audio codecName: " + tPMediaDecoderInfo.getDecoderName() + " maxSamplerate: " + tPMediaDecoderInfo.getMaxAudioSampleRate() + " maxChannels: " + tPMediaDecoderInfo.getMaxAudioChannels());
                ...
                TPNativeLog.printLog(4, "TPCodecUtils", "getAMediaCodecMaxCapabilityMap failed:" + e.getMessage());
            }
            return mMaxACodecHwCapabilityMap;
        }
    }
        ...
        if (!str.contains("audio")) {
            ...
            for (TPMediaDecoderInfo tPMediaDecoderInfo : tPMediaDecoderInfos) {
                if (TextUtils.equals(str, tPMediaDecoderInfo.getDecoderMimeType()) && tPMediaDecoderInfo.isSecureDecoder() == z) {
                    TPNativeLog.printLog(2, "TPCodecUtils", "getDecoderName:" + tPMediaDecoderInfo.getDecoderName());
    ...
    public static String getDisplayVersion() {
        ...
        try {
            ...
            TPNativeLog.printLog(4, "TPCodecUtils", "get huawei display version failed:" + e.getMessage());
            return null;
        }
    }
                ...
                if (TextUtils.equals(tPMediaDecoderInfo.getDecoderMimeType(), str)) {
                    for (TPMediaDecoderInfo.DecoderProfileLevel decoderProfileLevel : profileLevels) {
                        if (decoderProfileLevel.profile == convertDolbyVisionToOmxProfile) {
                            TPNativeLog.printLog(2, "TPCodecUtils", "getDolbyVisionDecoderName  profile:" + decoderProfileLevel.profile + " dvProfile:" + i + " bSecure:" + z + " name:" + tPMediaDecoderInfo.getDecoderName());
        ...
        synchronized (TPCodecUtils.class) {
            if (!(Build.VERSION.SDK_INT < 21 || i == 101 || i == -1)) {
                ...
                try {
                    ...
                    while (true) {
                        ...
                        if (supportedHWMimeType.equals(tPMediaDecoderInfo.getDecoderMimeType())) {
                            ...
                            if ((i3 <= i4 || (i3 <= decoderMaxWidth && i4 <= decoderLumaHeight)) && (i3 >= i4 || (i3 <= decoderLumaWidth && i4 <= decoderMaxHeight))) {
                                ...
                                TPNativeLog.printLog(2, "TPCodecUtils", "getSupportedFrameRatesFor max width:" + decoderMaxWidth + " max height:" + decoderLumaHeight + " max framerate for max resolution:" + decoderMaxFrameRateForMaxLuma + " current width:" + i3 + " height:" + i4 + " max support framerate:" + num);
                            } else {
                                TPNativeLog.printLog(4, "TPCodecUtils", "width:" + i3 + " height:" + i4 + " do not support! maxWidth:" + decoderMaxWidth + " maxHeight:" + decoderMaxHeight);
                                return 0;
                            }
                        } else {
                            i5++;
                    ...
                    TPNativeLog.printLog(4, "TPCodecUtils", "getMaxSupportedFrameRatesFor failed:" + e.getMessage());
                }
                return num.intValue();
            }
            return 30;
    ...
    public static String getSystemPatchVersion() {
        ...
        try {
            ...
            TPNativeLog.printLog(4, "TPCodecUtils", "get huawei system patch version failed:" + e.getMessage());
            return null;
        }
    }
    ...
    public static synchronized HashMap<Integer, TPCodecCapability.TPCodecMaxCapability> getVMediaCodecMaxCapabilityMap() {
        synchronized (TPCodecUtils.class) {
            TPNativeLog.printLog(2, "TPCodecUtils", "getVMediaCodecMaxCapabilityMap func in");
            if (!mMaxVCodecHwCapabilityMap.isEmpty()) {
                TPNativeLog.printLog(2, "TPCodecUtils", "return memory stored video max cap map");
            ...
            try {
                ...
                for (TPMediaDecoderInfo tPMediaDecoderInfo : tPMediaDecoderInfos) {
                    ...
                    if (tPMediaDecoderInfo.isVideo()) {
                        ...
                        TPNativeLog.printLog(2, "TPCodecUtils", "Video MimeType: " + decoderMimeType + " codecName: " + tPMediaDecoderInfo.getDecoderName());
                        if (isSupportedMediaCodec(decoderMimeType) && !tPMediaDecoderInfo.isSecureDecoder()) {
                            ...
                            TPNativeLog.printLog(2, "TPCodecUtils", "video codecName: " + tPMediaDecoderInfo.getDecoderName() + " lumasample: " + getMaxLumaSample(decoderMimeType, maxProfileLevel.level) + " framerate: " + tPMediaDecoderInfo.getDecoderMaxFrameRateForMaxLuma());
                            replace(Integer.valueOf(getSupportedCodecId(decoderMimeType)), tPCodecMaxCapability, mMaxVCodecHwCapabilityMap);
                        }
                    }
                }
            } catch (Exception e) {
                TPNativeLog.printLog(4, "TPCodecUtils", "getVMediaCodecMaxCapabilityMap failed:" + Log.getStackTraceString(e));
            }
            return mMaxVCodecHwCapabilityMap;
        }
    }
    ...
    public static synchronized void init(Context context, boolean z) {
        synchronized (TPCodecUtils.class) {
            TPNativeLog.printLog(2, "TPCodecUtils", "is local cache enabled:" + z);
                ...
                if (TextUtils.equals(tPMediaDecoderInfo.getDecoderMimeType(), "video/hevc")) {
                    for (TPMediaDecoderInfo.DecoderProfileLevel decoderProfileLevel : tPMediaDecoderInfo.getProfileLevels()) {
                        if (decoderProfileLevel.profile == i) {
                            TPNativeLog.printLog(2, "TPCodecUtils", "support hdr10 " + i);
                            return true;
                        }
                    }
                    continue;
    ...
    public static boolean isHDRDecoderTypeSupport(int i, int i2) {
        ...
        TPNativeLog.printLog(3, "TPCodecUtils", str);
        return false;
    }
    ...
    public static boolean isHDRsupport(int i, int i2, int i3) {
        ...
        TPNativeLog.printLog(2, "TPCodecUtils", "isHDRsupport(HDRVivid):display version:" + displayVersion);
        TPNativeLog.printLog(2, "TPCodecUtils", "isHDRsupport(HDRVivid):patch version:" + systemPatchVersion);
    ...
    private static boolean isInHDRVersionRangeWhiteList(ArrayList<TPCodecCapability.TPHdrSupportVersionRange> arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            if (Build.VERSION.SDK_INT <= tPHdrSupportVersionRange.upperboundAndroidAPILevel && Build.VERSION.SDK_INT >= tPHdrSupportVersionRange.lowerboundAndroidAPILevel) {
                TPNativeLog.printLog(2, "TPCodecUtils", "inHDRVersionRangeWhiteList!");
                return true;
            }
        }
        return false;
