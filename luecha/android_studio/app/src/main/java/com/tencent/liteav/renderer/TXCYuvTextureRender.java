...
public class TXCYuvTextureRender {
    ...
    public TXCYuvTextureRender() {
        ...
        this.mTextureBuffer = ByteBuffer.allocateDirect(fArr.length * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
    ...
    private void reloadFrameBuffer() {
        if (this.mNeedReLoadFrameBuffer) {
            String str = TAG;
            TXCLog.i(str, "reloadFrameBuffer. size = " + this.mWidth + ProxyConfig.MATCH_ALL_SCHEMES + this.mHeight);
            destroyFrameBuffer();
            int[] iArr = new int[1];
            int[] iArr2 = new int[1];
