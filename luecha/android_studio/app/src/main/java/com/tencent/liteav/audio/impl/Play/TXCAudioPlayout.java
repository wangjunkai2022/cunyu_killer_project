...
public class TXCAudioPlayout {
    ...
    public boolean startPlayout(int i, int i2, int i3, int i4) {
        ...
        if (minBufferSize <= 0) {
            TXCLog.e("TXCAudioPlayout", "AudioTrack.getMinBufferSize returns invalid value: " + minBufferSize);
            return false;
        }
        ...
        try {
            ...
            try {
                if (audioTrack.getState() == 1) {
                    audioTrack.play();
                    TXCLog.i("TXCAudioPlayout", "create AudioTrack success. samplerate: %d, channelConfig: %d, bufferSize: %d, streamType: %s", Integer.valueOf(i), Integer.valueOf(i2), Integer.valueOf(i3), streamTypeToText(i4));
                    return audioTrack;
                }
                throw new RuntimeException("AudioTrack is not initialized.");
            } catch (Exception unused) {
                TXCLog.w("TXCAudioPlayout", "create AudioTrack failed. samplerate: %d, channelConfig: %d, bufferSize: %d, streamType: %s", Integer.valueOf(i), Integer.valueOf(i2), Integer.valueOf(i3), streamTypeToText(i4));
    ...
    private static void tearDownAudioTrack(AudioTrack audioTrack) {
        if (audioTrack != null) {
            try {
                ...
                TXCLog.e("TXCAudioPlayout", "stop AudioTrack failed.", e);
            }
        }
    }
    ...
    public int write(ByteBuffer byteBuffer, int i, int i2) {
        ...
        TXCLog.e("TXCAudioPlayout", "write audio data to AudioTrack failed. " + i3);
        return -1;
    }
