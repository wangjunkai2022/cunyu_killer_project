...
public final class DashUtil {
    public static DataSpec buildDataSpec(String str, RangedUri rangedUri, String str2, int i) {
        return new DataSpec.Builder().setUri(rangedUri.resolveUri(str)).setPosition(rangedUri.start).setLength(rangedUri.length).setKey(str2).setFlags(i).build();
    }

    public static DataSpec buildDataSpec(Representation representation, RangedUri rangedUri, int i) {
        return buildDataSpec(representation.baseUrls.get(0).url, rangedUri, representation.getCacheKey(), i);
    }
    ...
    private static void loadInitializationData(DataSource dataSource, Representation representation, int i, ChunkExtractor chunkExtractor, RangedUri rangedUri) throws IOException {
        new InitializationChunk(dataSource, buildDataSpec(representation.baseUrls.get(i).url, rangedUri, representation.getCacheKey(), 0), representation.format, 0, null, chunkExtractor).load();
    }
