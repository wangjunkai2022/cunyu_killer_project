...
public class FlutterJNI {
    ...
    public static Bitmap decodeImage(ByteBuffer byteBuffer, long j) {
        if (Build.VERSION.SDK_INT >= 28) {
            try {
                return ImageDecoder.decodeBitmap(ImageDecoder.createSource(byteBuffer), new ImageDecoder.OnHeaderDecodedListener(j) { // from class: io.flutter.embedding.engine.-$$Lambda$FlutterJNI$fKNoB8FUFQGoFikQU9q4OZFIOmE
                ...
                Log.e("FlutterJNI", "Failed to decode image", e);
            }
        }
        return null;
    }
    ...
    public void invokePlatformMessageEmptyResponseCallback(int i) {
        ...
        try {
            if (isAttached()) {
                ...
                Log.w("FlutterJNI", "Tried to send a platform message response, but FlutterJNI was detached from native C++. Could not send. Response ID: " + i);
            }
        } finally {
